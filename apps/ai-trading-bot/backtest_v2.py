"""
backtest_v2.py — Production backtesting pipeline for StockTransformerV2.
"""
...
from utils.trading_v2 import generate_signal_v2, CONFIDENCE_FLOOR  # works now
...
def backtest_v2(
    ...
    for i in range(len(dataset)):
        ...
        capital += pnl
        trade_returns.append(pnl / max(capital - pnl, 1e-9))
        capital_curve.append(capital)
    ...
    results = {
        ...
        "total_trades":      total,
        "held":              held_count,
        "trade_rate_pct":    round(total / max(total + held_count, 1) * 100, 1),
        ...
        "strong_accuracy":   round(strong_wins / strong_total, 4) if strong_total else 0.0,
        "medium_trades":     medium_total,
        "medium_accuracy":   round(medium_wins / medium_total, 4) if medium_total else 0.0,
...
def _annualised_sharpe(returns: list) -> float:
    if len(returns) < 2:
        return 0.0
    arr = np.array(returns, dtype=np.float64)
    std = arr.std()
    return float(np.sqrt(252) * arr.mean() / std) if std > 1e-9 else 0.0
...
def _print_results(r: dict):
    ...
    print(f"  Total Trades:         {r['total_trades']:>6}  (held {r['held']}, traded {r['trade_rate_pct']:.1f}%)")
    print(f"  Accuracy:             {r['accuracy']:>6.2%}")
    print(f"  Sharpe Ratio:         {r['sharpe_ratio']:>6.3f}")
    print(f"  Max Drawdown:         {r['max_drawdown_pct']:>6.2f}%")
    print(f"  Best Win Streak:      {r['best_win_streak']:>6}")
    print(f"  Worst Loss Streak:    {r['worst_loss_streak']:>6}")
    if r["strong_trades"]:
        print(f"  STRONG trades:        {r['strong_trades']:>6}  acc={r['strong_accuracy']:.2%}")
    if r["medium_trades"]:
        print(f"  MEDIUM trades:        {r['medium_trades']:>6}  acc={r['medium_accuracy']:.2%}")
    print(f"{sep}\n")
...
if __name__ == "__main__":
    ...
    cfg = torch.load(args.config, map_location="cpu") if os.path.exists(args.config) else {}
    model = StockTransformerV2(
        input_dim=cfg.get("input_dim", dataset.n_features),
    ...
    backtest_v2(model, dataset, min_confidence=args.confidence, device=args.device)