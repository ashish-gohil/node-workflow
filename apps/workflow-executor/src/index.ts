...
export const handler = async (event: SQSEvent) => {
  ...
  const workflows: { workflowId: string; executionId: string }[] = event.Records.map((record) =>
  ...
  if (workflows.length > 0) {
    for (const wf of workflows) {
      ...
      console.log(
        `Executing workflow with workflowId, ${wf.workflowId} and executionId, ${wf.executionId}`
      );
