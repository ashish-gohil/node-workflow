  ...
  return (
    <Sheet open={triggerSheetOpen} onOpenChange={setTriggerSheetOpen}>
      <SheetTrigger asChild>
        <Button
          variant="ghost"
          className="bg-bg-surface hover:bg-bg-overlay border-border-strong hover:border-border-intense text-text-secondary hover:text-text-primary relative z-30 rounded-sm border-2 border-dashed p-10 transition-colors duration-[150ms] hover:translate-none hover:transform-none"
        >
          <svg
            xmlns="http://www.w3.org/2000/svg"
            fill="none"
            ...
            {triggerNodes.map((node) => (
              <div
                onClick={() => {
                  ...
                  const newNode: TriggerNode = createTriggerNode(node.type);
