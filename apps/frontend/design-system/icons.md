...
<button class="btn-secondary">
  <svg class="size-4">...</svg>  <!-- inherits text-text-primary -->
  Filter
</button>
```
...
<button aria-label="Delete workflow"
        class="size-9 inline-flex items-center justify-center rounded-sm
...
### Spinner (loading)
```html
<svg class="size-4 animate-spin text-forest-300" viewBox="0 0 16 16"
...
Rotate 90° on expand:
```html
<svg class="size-4 transition-transform duration-fast
